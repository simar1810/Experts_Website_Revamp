...
function LocationSelectorDropdown({
  coordinateLocation,
  locationQuery,
  setLocationQuery,
  ...
  return (
    <div
      className={`absolute top-full left-0 right-0 mt-2 rounded-2xl shadow-xl p-4 z-999 border ${
...
function SpecialitySelectorDropdown({
  ...
  const filtered = availableSpecialities.filter(
    (spec) =>
...
export default function SearchFilters({
  ...
  const handleUseMyLocation = async () => {
    if (coordinateLocation && setClientLocation) {
      const { latitude, longitude } = coordinateLocation;
      setClientLocation({
        type: "Point",
  ...
  return (
    <div
      className={`flex flex-col lg:flex-row items-stretch gap-1 ${containerClassName}`}
      style={{ position: "relative", zIndex: 10 }}
    >
      {/* Speciality Input */}
      <div
        className={`flex items-center flex-1 px-3 py-1 min-w-0 ${inputWrapperClassName}`}
      ...
      >
        ...
        <div className="flex flex-wrap gap-1.5 flex-1 items-center min-w-0">
          ...
          <input
            ...
            onBlur={() =>
              setTimeout(() => setShowSpecialityDropdown(false), 200)
            }
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
      ...
      >
        ...
        {coordinateLocation && (
          <button
            ...
            className={`flex justify-center items-center gap-x-1 ml-2 shrink-0 text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-full border transition-colors ${
              isDark
                ? "border-gray-700 text-gray-300 hover:border-lime-500 hover:text-lime-400"
                : "border-gray-200 text-gray-500 hover:border-lime-400 hover:text-lime-700"
            }`}
          >
