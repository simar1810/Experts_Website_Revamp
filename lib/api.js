...
/** First path segment on shop.* must not be sent as x-tenant (e.g. /dashboard → "dashboard"). */
const SHOP_NON_TENANT_SEGMENTS = new Set([
  "dashboard",
  "profile",
  "find-experts",
  "discover-programs",
...
function resolveTenantFromPathname(pathname) {
  ...
  if (SHOP_NON_TENANT_SEGMENTS.has(tenant)) return null;
  return tenant;
}
