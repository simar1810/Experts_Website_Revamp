...
function zeefitExpertImage(n) {
  return `/images/zeefit/${encodeURIComponent(`experts ${n}.jpg`)}`;
}

/** Hero crossfade — files under `public/images/zeefit/experts N.jpg`. */
export const discoverHeroSlides = [1, 2, 3, 4, 5].map((n) => ({
  src: zeefitExpertImage(n),
  alt: "Zeefit wellness and coaching programs",
}));

export const discoverHeroContent = {
  badge: "CURATED WELLNESS",
  titleBefore: "Explore Proven fitness Programs, Designed by",
  titleHighlight: "Top Fitness Experts",
  description:
    "Find proven fitness programs created by trusted experts, so you can stop guessing and start seeing real progress.",
  /** ms between slide changes (fade duration set in `DiscoverHeroSlideshow`). */
  heroSlideIntervalMs: 5500,
};
