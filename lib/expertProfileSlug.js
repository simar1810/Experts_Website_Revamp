...
async function resolveListingFromProfilePathViaBackend(
  locationSlug,
  specSlug,
  nameSegment,
) {
  try {
    const res = await fetch(
      `${API_BASE}/experts/listing/resolve-profile-path`,
      {
        ...
        body: JSON.stringify({ locationSlug, specSlug, nameSegment }),
        cache: "no-store",
      },
    );
    ...
    return {
      ...
      contentPendingReview: Boolean(data.contentPendingReview),
    };
  } catch {
    return null;
  }
...
/** Resolve profile URL segments to a listing (search index first, then DB fallback). */
export async function resolveListingFromProfilePath(
  locationSlug,
  specSlug,
  nameSegment,
) {
  ...
  if (pick) {
    return { listingId: String(resolveListingId(pick)), approved: true };
  }
  return resolveListingFromProfilePathViaBackend(
    locationSlug,
    specSlug,
    nameSegment,
  );
