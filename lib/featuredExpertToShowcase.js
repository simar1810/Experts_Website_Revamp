...
export function featuredStaticExpertToCoachShowcase(expert, stableIndex = 0) {
  ...
  /** Use Next/Image optimizer for static assets (`/images/...`) and Unsplash; bypass only uncommon remote CDs without patterns. */
  const shouldBypassOptimizer =
    /^https:\/\//i.test(imageSrc) &&
    !/^https:\/\/images\.unsplash\.com\//i.test(imageSrc);
  ...
  return {
    ...
    websiteLink,
    imageSrc,
    imageAlt: `${rawName} — wellness coach`,
    listingHref,
    imageUnoptimized: shouldBypassOptimizer,
  };
}
