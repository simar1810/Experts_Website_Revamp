...
/**
 * Preview payable amount for checkout (product marketing coupon or coach affiliate code).
 * Coach commission % is resolved on the server as a share of WellnessZ platform commission
 * (from the partner–coach assignment).
 */
...
/** Optional suffix for card UI (e.g. "/month") from product metadata. */
export function getProductPriceSuffix(product) {
  const metadata = product?.metadata && typeof product.metadata === "object"
    ? product.metadata
    : {};
  const period = cleanString(
    metadata.billingPeriod || metadata.pricePeriod || metadata.priceSuffix,
  ).toLowerCase();
  if (period === "month" || period === "monthly" || period === "/month") {
    return "/month";
  }
  if (period === "year" || period === "yearly" || period === "/year") {
    return "/year";
  }
  return "";
}
...
export function getProductTechnicalItems(product) {
  ...
  return items
    .map((item, index) => {
      ...
      const icon = cleanString(item.icon ?? item.iconName ?? "");
